...
```{r}
# # Candlestick pathern of the stock 
charting_data <- returns_long %>% filter(Ticker == ticker,Date >= 	"2020-01-01")
...
```
```{r}
returns_long %>% filter(Ticker == ticker)
```

```{r}
# calculate the Moving Average on the Adj_close using TTR package 
Moving_Average_data<-returns_long %>% filter(Ticker == ticker)%>%
        mutate(SMA100=SMA(Adj_Close,n=100),
               SMA50=SMA(Adj_Close,n=50),
               Zone=ifelse(SMA50>=SMA100,"buy signal","sell signal")) %>%
  select("Date","Ticker", "Open", "High", "Low", "Close", "Adj_Close", "Volume","SMA100","SMA50","Zone")
Moving_Average_data
```

```{r}
# Kan7awl nrssm l moving average!!
Moving_Average_data %>% filter(Date >= 	"2020-01-01") %>%
  plot_ly(
  type="candlestick",
  x=~Date,
  open=~Open,
  ...
  high=~High,
  low=~Low
) %>% add_trace(y = ~SMA100, name = 'SMA100', width = 4,type = 'heatmap')
```
```{r}
